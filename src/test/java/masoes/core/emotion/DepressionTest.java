...
public class DepressionTest {
    ...
    public void shouldIntersectsWithBoundaryPoints() {
        Arrays.stream(coordinates)
                .forEach(coordinate -> assertTrue(depression.getGeometry().intersects(geometryCreator.createPoint(coordinate.x, coordinate.y))));
    }

    @Test
    public void shouldContainsInsidePoint() {
        assertTrue(depression.getGeometry().intersects(geometryCreator.createPoint(-0.7, -0.7)));
        assertTrue(depression.getGeometry().intersects(geometryCreator.createPoint(-0.51, -0.51)));
    }

    @Test
    public void shouldNotContainsPoint() {
        assertFalse(depression.getGeometry().intersects(geometryCreator.createPoint(-0.1, -0.1)));
        assertFalse(depression.getGeometry().intersects(geometryCreator.createPoint(-1.1, -0.1)));
    }
